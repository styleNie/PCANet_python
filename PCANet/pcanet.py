...
def PCA_output(InImg,InImgIdx,patchsize,NumFilters,V):
    ...
    for i in range(ImgZ):
        ...
        for j in range(NumFilters):
            OutImg.append( V[:,j].T.dot(im).reshape(ImgX,ImgY,1L) )
            cnt+=1
...
def PCANet_train(InImg,PCANet,IdxExt):
    ...
    if IdxExt==1:
        ...
        for idx in range(NumImg):
            ...
            OutImg_i,ImgIdx_i=PCA_output(OutImg_tmp, zeros((len(OutImgIndex),1),dtype='int'),PCANet.PatchSize[-1], PCANet.NumFilters[-1], V[-1])
